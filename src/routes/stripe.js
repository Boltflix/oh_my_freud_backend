...
// IDs de preço (compat com chaves antigas)
const PRICE_MONTHLY =
...
function planFrom(req) {
  const bodyPlan = req.body && (req.body.plan || req.body["plan"]);
  const queryPlan = req.query && (req.query.plan || req.query["plan"]);
  return bodyPlan || queryPlan || null;
}
...
router.get("/config", (req, res) => {
  res.json({
    ...
    hasStripeKey: !!SECRET,
    monthly: {
      id: PRICE_MONTHLY || null,
      source:
        (process.env.STRIPE_MONTHLY_PRICE_ID && "STRIPE_MONTHLY_PRICE_ID") ||
        (process.env.STRIPE_PRICE_MONTHLY && "STRIPE_PRICE_MONTHLY") ||
        (process.env.STRIPE_PRICE_ID && "STRIPE_PRICE_ID") ||
        null,
    },
    annual: {
      id: PRICE_ANNUAL || null,
      source:
        (process.env.STRIPE_ANNUAL_PRICE_ID && "STRIPE_ANNUAL_PRICE_ID") ||
        (process.env.STRIPE_PRICE_ANNUAL && "STRIPE_PRICE_ANNUAL") ||
        null,
    },
